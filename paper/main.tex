...
\subsection{Mechanism: shunting inhibition cancels itself}
\label{sec:shunt}
...
\subsection{Three parameterisations compared}
\label{sec:threeway}
...
(ii) per-receptor, global & published physiology & $-0.0356$ &: & pending & pending\\
(iii) uniform $\times2$, global & search & $-0.0533$ &: & \textbf{fail} &: \\
...
pool to $168$\,Hz instead of suppressing it.}
\label{tab:threeway}
\end{table}
...
\subsection{Behaviour under regional parameterisation}
\label{sec:behaviour}
...
The optimum is also narrow, selectivity collapsing by a factor of four, which
is characteristic of maximisation over a one-dimensional parameter slice. A
structured search over per-receptor and regional parameters jointly is
therefore the appropriate next step, and configuration (iv) should be read as a
lower bound on what regional parameterisation can achieve.
