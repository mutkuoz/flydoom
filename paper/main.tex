...
A large language model (Anthropic Claude, used through Claude Code) contributed
substantially to this work: it wrote much of the simulator, the experiment
harness and the analysis scripts, and it drafted and revised the text of this
manuscript. The author designed the study, fixed its constraints, directed the
investigation, and reviewed and corrected the generated code and prose.

Two boundaries are worth stating because they bear on how the results should be
read. First, no quantity reported here was produced by a language model: every
number comes from executing the released code, and every table and figure is
emitted by a script from the serialized run data rather than transcribed, so
the two cannot diverge. Second, the model is not an author and takes no
responsibility for the contents; the author takes full responsibility for all
of them, irrespective of how they were generated.
