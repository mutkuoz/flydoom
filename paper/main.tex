...
synapse downstream ($-17.7\pm7.4$\,Hz). None of this was fitted and none
survives a degree-preserving shuffle. Per-cell direction selectivity is not
reproduced: $|\mathrm{DSI}|\leq0.0121$, roughly $40\times$ below the threshold
at which experimental studies classify a terminal as direction-tuned.

The obstacle is a property of the neuron model. A product requires a
multiplicative interaction between the correlator arms, which in a
conductance-based neuron must arise from inhibitory division, and that division
cancels itself: shunting divides the membrane by $1.77\times$ at \cell{T4a}
while the accompanying hyperpolarisation raises the excitatory driving force by
$1.85\times$, leaving an interaction below 11\% and of the wrong sign. Making
inhibition purely divisive abolishes the residue rather than restoring it, so
what survives is a threshold conjunction and not a product.

Raising the inhibitory-to-excitatory ratio restores the interaction but no
single value serves the brain: applied globally it drives the proboscis pool to
168\,Hz under bitter, because 51\% of inhibitory synapses terminate on
inhibitory neurons. Restricted to the visual populations it leaves the
chemosensory results intact and raises selectivity sevenfold. Published
per-receptor ratios under the same restriction, which fit nothing, recover most
of that gain, isolating scope rather than the values as the operative variable.

Recording site proves more consequential than any of these parameters. Read at
the horizontal system, the optomotor output stage in the animal, the frozen
model carries a directional signal of 17 to 26\,Hz that ablating
\cell{T4}/\cell{T5} abolishes, an order of magnitude above anything resolvable
at the single descending pair used as the steering readout. The regional scale
that maximises per-cell selectivity drives that signal to zero or below: the
two measures move against each other, and the parameter search optimised the
one behaviour cannot use. Crossing the neuron transfer function with the scale
over thirty held-out seeds separates them completely, conditions differing by
an order of magnitude and in sign at the output stage returning four and six of
eighteen measures against a command-matched control. The signal exists and does
not reach the readout. We withdraw the vision--steering correlation as evidence
of vision, since it tracks the inhibitory scale in both transfer functions
rather than the directional signal that scale abolishes.

Two methodological results generalise beyond this model: degree-preserving
shuffles are invalid as controls in closed loop, because the arms sample
different stimulus distributions; and episode-level behavioural measures in a
stochastic environment failed to resolve a population signal varying by an
order of magnitude across conditions, which bounds what any closed-loop
connectome study can claim from them.
\end{abstract}
...
for this question. What can be said is
