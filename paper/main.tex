...
produce no directional response. The advantage replicates in a second arena
over sixty seeds once every lens sees the scene, and is absent there with the
original field of view. Two errors in the visual interface, an eye map read
with the wrong hexagonal convention and a reversed steering sign, are corrected
and measured: together they double the population direction signal and still
produce no optomotor response.
...
(0\% reversed) and roughly doubles exploration over three levels; the
behavioural tables have not yet been re-run with it.
