...
\title{\bf A frozen whole-brain \emph{Drosophila} connectome\\[2pt]
in a closed sensorimotor loop with \emph{Doom}}

\author{Mehmet Utku \"Ozt\"urk\\[2pt]
...
\begin{abstract}
\noindent
The synapse-resolution \emph{Drosophila} connectome makes it possible to
simulate an entire brain with essentially no free parameters. Whole-brain
models of this kind are at present driven \emph{open loop}: sensory neurons are
stimulated and motor neurons are read. We embed a frozen FlyWire FAFB v783
connectome (139{,}255 neurons; 2{,}710{,}038 edges at a five-synapse threshold)
in a \emph{closed} sensorimotor loop with \emph{Doom}, running at $1.01\times$
realtime on one consumer GPU. Nothing is trained; the
only fitted quantity is a single global synaptic gain, calibrated on a
non-visual behaviour.

We find a sharp dissociation between modalities. Chemosensory transformations
transfer: sugar stimulation drives the proboscis motor pool to 77\,Hz, bitter
silences it, and co-stimulation is 99\% suppressed --- none of which was fitted.
Adding an olfactory channel raises lateral horn activity from 0.32 to
$14.1\pm5.5$\,Hz in $10/10$ seeds ($p=0.002$, two-sided sign test) and changes
descending-neuron output one synapse downstream, with the sign the wiring
predicts (\cell{LH}$\rightarrow$\cell{DNp01}, $-248$ synapses, suppression;
$-17.7\pm7.4$\,Hz, $9/10$ seeds, $p=0.021$), while a degree-preserving shuffle
of the same graph produces no such effect.

Visual motion computation does not transfer. Direction selectivity and looming
discrimination both fail, and we show the failure is a property of the model
class rather than of the connectome: under linear synaptic summation with a
single output threshold, $\mathrm{thr}(A+B)$ is symmetric in $A$ and $B$, so
temporal order is not recoverable. The correlator geometry is verifiably present
in the graph and a retinotopic travelling wave reaches \cell{T4a}, yet six
candidate remedies --- per-type conduction delays, heterogeneous membrane time
constants, conductance-based shunting, a forced high-conductance state,
population pooling, and graded non-spiking optic-lobe units --- yield at best
$|\mathrm{DSI}|=0.0121$, some $40\times$ below the threshold conventionally
used to \emph{select} direction-selective terminals experimentally, with a
mirror-pair sign relation no better than chance across the operating range. The residual
obstacle is measurable: \cell{T4a}'s fast excitatory arm is starved $37\!:\!1$ by
glutamatergic \cell{L1} inhibition.

Read against connectome-constrained models whose unknown parameters are
optimized, and which do recover motion selectivity, this localizes what that
optimization contributes: the wiring is common to both, so what optimization
supplies is the per-type gain and timing structure that balances the
correlator's arms. We additionally report a methodological result --- a
degree-preserving shuffle control is invalid in closed loop, because the two
arms generate different stimulus distributions.
\end{abstract}
...
\subsection{A frozen whole-brain connectome running in closed loop}
...
\subsection{Chemosensory transformation transfers}
...
\subsection{Visual motion computation does not transfer}
...
\paragraph{The looming detector is not weakly driven; it is held shut.}
...
\subsection{Why motion fails: temporal order, and a starved arm}

\begin{figure}[t]\centering
\includegraphics[width=\textwidth]{figs/fig2_mechanism.pdf}
\caption{Why motion fails. \textbf{(a)} A unit that sums its inputs and applies
one threshold to the sum cannot recover order: ``$A$ then $B$'' and ``$B$ then
$A$'' produce identical sums (rust curve, with the second case overlaid dotted),
so any threshold on that sum responds identically. \textbf{(b)} Measured firing
...
In the model, presynaptic inputs sum linearly into a conductance and the only
nonlinearity is a threshold on that sum. But $\mathrm{thr}(A+B)$ is symmetric
in its arguments, so a unit that sums its inputs and thresholds the sum cannot
distinguish the two cases. This is not a tuning deficiency; it is a structural
property of the model class. Biological elementary motion detectors escape it
by delaying one arm and combining the arms
...
null-direction suppression~\citep{haag2016,gruntman2018}.
...
The residual obstacle is measurable and specific: the fast arm is starved.
...
\subsection{Olfactory valence transfers, and beats a shuffled control}
...
We want to be precise about what does and does not discriminate here. Sign
consistency does \emph{not}: several shuffled populations remain fairly
...
we read it that way. We also note that the shuffled \cell{DNp01} residual
...
than of a transformation. The honest summary is that the intact effects are
large and reproducible while the shuffled ones sit at the control's floor.
\emph{The effect requires the wiring.}

The \cell{BPN} and \cell{MDN} walking effects deserve a weaker claim than the
...
${\sim}100\times$, and we would not rest anything on them alone.
...
Two caveats attach permanently. Odour source distances are read from the
...
\paragraph{A path that exists is not a path that carries.}
...
\subsection{Shuffle controls are invalid in closed loop}
\label{sec:shuffle}
...
We measured this directly. Over a matched episode the intact agent encountered
an enemy on 186 of 500 tics and finished at full health; the shuffled agent
encountered one on 489 of 492 tics and finished at 16 health. The two arms
...
This is worth stating carefully, because it is easy to over-read as a claim
about connectomics. The connectome contains the correlator: we found the
geometry, the mirrored axes, and the travelling wave that drives them. What it
does not contain, and cannot, is the per-type gain and timing structure that
makes the two arms comparable in magnitude. In our hands the arms differ by
$37\times$, and the ${\sim}40\times$ subthreshold, sign-unstable residue that
survives is what a correlator produces when one of its inputs is whispering.
...
measurement of what the optimization supplies --- and our arm-imbalance analysis
suggests a concrete hypothesis about what that is: relative gain along the ON
pathway's disinhibitory chain. That hypothesis is testable by inspecting the
learned parameters of the optimized model, which we have not done and which
would be the natural next step.

Two further readings we want to foreclose. First, none of this is a claim about
...
\paragraph{Data.} FlyWire FAFB v783~\citep{dorkenwald2024,schlegel2024},
...
\paragraph{Neuron model.} Leaky integrate-and-fire with parameters as listed in
...
\paragraph{Departures from the reference model.} Each is switchable and each
...
\paragraph{The environment, and what we changed about it.} Modifying the
...
not a route either: \texttt{vid\_gamma}, \texttt{vid\_contrast},
\texttt{vid\_brightness} and \texttt{vid\_saturation} leave the rendered buffer
...
\paragraph{Rate readout.} An exponential filter over per-step output.
...
\paragraph{Statistics.} The olfaction comparison uses 10 independent
...
environment into the fly's band; and the engine's own rendering controls. The
common structure of these failures is that each is a scale applied to both arms
of an imbalance, and a scale cancels from a ratio.
...
\emph{Deferred.} Per-cell-type synaptic gains are the identified obstacle and
we do not fit them, because fitting them is the independent variable of this
study rather than a repair to it. We note the target is not a single set of
...
licence. The connectome data are CC BY-NC 4.0, are not redistributed, and must
be obtained from FlyWire directly; a fetch script is provided. The environment
uses ViZDoom~\citep{kempka2016} with the freely licensed Freedoom asset set, so
no commercial game data are required.
