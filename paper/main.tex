...
synapse downstream ($-17.7\pm7.4$\,Hz). None of this was fitted and none of it
survives a degree-preserving shuffle. Visual motion computation is not
...
by $g_{\mathrm{tot}}$ ($1.77\times$ at \cell{T4a}) while the hyperpolarisation
it causes raises the excitatory driving force by $1.85\times$, leaving an
interaction below 11\% and of the wrong sign. The cell sums, and summation
cannot be direction selective at any wiring. Making inhibition purely divisive
abolishes what selectivity remains rather than restoring it: the residue is a
threshold conjunction, not a product.

Increasing the inhibitory-to-excitatory ratio restores the interaction.
Applied globally it inverts the gustatory result, driving the proboscis pool to
168\,Hz under bitter, because 51\% of inhibitory synapses terminate on
inhibitory neurons. Restricted to the visual populations the same factor leaves
the gustatory circuit numerically unchanged, preserves olfaction, and raises
direction selectivity sevenfold. It does
not, however, produce closed-loop behaviour. On thirty held-out environment
seeds the regionally scaled model separates from a command-matched control on
six of eighteen measures, the same number as the frozen model, and the two are
not distinguishable by that criterion. Only locomotion statistics change: against the frozen model on identical
seeds, collisions fall by $3.95\pm3.79$ per 1{,}000 tics and net displacement
rises by $131\pm102$, while health collected, the task objective, does not.

The published ratios under that same regional restriction, which fits nothing,
recover most of the gain and preserve both chemosensory results, isolating
scope rather than the values. Over all parameters that do not entail
per-cell-type fitting the largest selectivity found was $0.0792$.
...
All four parameterisations were run through the identical protocol on the same
thirty environment seeds (40--69), none of which was used while selecting
parameters. Each is compared against its own command-matched AR(1) control,
paired by seed, because seed-to-seed variance in this environment is far larger
than the differences under test. A metric counts as separating when the paired
difference has a 95\% confidence interval excluding zero.
...
does. The second column counts only metrics with a defined better direction:
three of the eighteen (path length, and mean yaw and forward magnitude)
describe what the agent did rather than how well, and are excluded rather than
assigned a sign. Restoring the multiplicative interaction therefore raises
direction selectivity sevenfold without producing task-level behaviour.

The direct comparison is more sensitive than either against a control, since
the regional and frozen models see identical seeds. Two of nine task metrics
move: collisions fall by $3.95\pm3.79$ per 1{,}000 tics and net displacement
rises by $131\pm102$ units, both favouring the regional model. Health
collected, which is the objective in this scenario, does not improve
($-6.53\pm7.81$), and against its own control the regional model collects
\emph{less} than the control does ($-7.73\pm8.86$). The vision--steering
correlation separates the two models ($+0.25\pm0.16$) but moves in the
direction opposite to optomotor steering, from $-0.090$ frozen to $+0.161$
regional. The defensible reading is that the manipulation changes how the model
moves without changing how well it does the task.
...
Two readings of that are available and we cannot presently separate them. The
manipulation may be too small: selectivity after the fix remains an order of
magnitude below the experimental selection threshold, and a system that far
from the animal's operating point need not steer. Or the measurement may be
too coarse: health collected varies by roughly 13 units across seeds against
an effect of a few units, which is why the power analysis puts the required
sample near $n\approx106$ and why two runs of one configuration returned 3.2
and 11.2. The second is not a dismissal of the first. What can be said is
