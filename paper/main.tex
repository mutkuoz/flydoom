...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[LE]{\small\itshape \"Ozt\"urk}
\fancyhead[RO]{\small\itshape A frozen connectome in closed loop}
...
\title{A frozen whole-brain \emph{Drosophila} connectome\\[3pt]
...
\begin{abstract}
\noindent
The synapse-resolution \emph{Drosophila} connectome permits simulation of an
entire brain with essentially no free parameters, but such models are at
present driven open loop: sensory neurons are stimulated and motor neurons are
read. We embed a frozen FlyWire FAFB v783 connectome (139{,}255 neurons;
2{,}710{,}038 edges at a five-synapse threshold) in a \emph{closed}
sensorimotor loop with a video game, at $0.19\times$ realtime on one
consumer CPU. Nothing is trained, and the only fitted quantity is a single
global synaptic gain calibrated on a non-visual behaviour. We find a
dissociation between modalities. Chemosensory transformations transfer: sugar
stimulation drives the proboscis motor pool to 77\,Hz and co-stimulation with
bitter is 99\% suppressed, neither of which was fitted, and an added olfactory
channel raises lateral horn activity from 0.32 to $14.1\pm5.5$\,Hz and
suppresses a descending neuron one synapse downstream with the sign the wiring
predicts ($-17.7\pm7.4$\,Hz), while a degree-preserving shuffle of the same
graph does not. Visual motion computation does not transfer. Direction
selectivity and looming discrimination both fail: across an operating-point
sweep $|\mathrm{DSI}|\leq0.0121$, some $40\times$ below the value at which
experimental studies merely \emph{select} a terminal as direction-tuned, and
with a mirror-pair sign relation no better than chance. An earlier version of
this manuscript attributed that failure to the model class, arguing that
$\mathrm{thr}(A+B)$ is symmetric in $A$ and $B$ and that temporal order is
therefore not recoverable. We withdraw the argument. It describes a
\emph{memoryless} unit, whereas a two-input correlator built from the same
neuron model in the same simulator, differing only in that one arm is delayed,
reaches $\mathrm{DSI}=-0.79\pm0.017$; conduction delays and synaptic time
constants supply exactly the memory the argument omits. The model class is not
the obstacle. The correlator geometry is verifiably present in the graph, and
the arms are measurably imbalanced --- \cell{T4a}'s fast excitatory arm is
starved $37\!:\!1$, and the looming detector \cell{LPLC2} sits below its
resting potential under roughly tenfold inhibition --- yet neither rebalancing
the arms with short-term depression nor transplanting per-type gains from an
optimized model raises selectivity; both lower it. Candidate mechanisms
spanning pooling, sampling geometry, phase offset, saturation, operating point
and arm imbalance were tested and eliminated one by one. The obstacle is then identified and it is
a property of the model, not of the connectome. A correlator's multiplication
must come from inhibition dividing the membrane, and that division cancels
itself: the shunt divides by $g_{\mathrm{tot}}$ ($1.77\times$ at \cell{T4a})
while the hyperpolarisation it causes raises the excitatory driving force by
$1.85\times$, leaving under $11\%$ interaction with the wrong sign. The cell
performs addition, which no wiring can make direction selective. Raising the
inhibitory-to-excitatory ratio restores the multiplication --- but doing so
globally destroys the gustatory result, inverting bitter from suppressing the
proboscis motor pool to driving it at $168$\,Hz, because $51\%$ of inhibitory
synapses terminate on other inhibitory neurons. Applied to the visual
populations alone, the identical factor leaves the gustatory circuit
byte-identical and olfaction intact, raises direction selectivity sevenfold,
and produces behaviour: on thirty held-out environment seeds the model
separates from a command-matched control on ten of eighteen measures --- more
ground covered, less time immobilised, fewer collisions --- where the frozen
model separates on none. A single global synaptic gain is therefore not merely
incomplete but inconsistent, since the optic lobe requires a conductance ratio
the SEZ cannot survive; published physiology places that requirement in the
same range.
Read against connectome-constrained models whose unknown parameters are
optimized, the wiring is common to both, so what optimization supplies is some
combination of per-type gain and timing that we can bound but cannot yet
identify --- transplanting its gains alone does not suffice. We additionally report a
methodological result --- a degree-preserving shuffle control is invalid in
closed loop, because the two arms generate different stimulus distributions.
\end{abstract}
...
(139{,}255 neurons, single consumer GPU or CPU), released as open code. An
earlier version of this manuscript described the loop as \emph{real time} at
$1.01\times$; that figure is wrong by a factor of 17 and is corrected below.
\item A modality dissociation: chemosensory transformations transfer, visual
motion computations do not.
\item A mechanistic account of the visual failure, with six candidate remedies
implemented and measured, isolating a $37\!:\!1$ imbalance in the elementary
motion detector's two arms.
...
precisely the input a correlator tuned to a fixed spacing cannot use. The two eyes are splayed $\pm40^\circ$; collapsing them onto the
screen centre makes both eyes see the same image and drives the left--right
steering difference to zero by construction. The engine's field of view must be set through the
...
so that circuit is absent. Nor is there a state signal to substitute for it. \cell{AOTU042} suppresses
...
\subsection{Diagnosing the motion failure}
\label{sec:mech}
...
An earlier version of this manuscript argued that the failure was structural.
In the model presynaptic inputs sum into a conductance and the only
nonlinearity is a threshold on that sum; since $\mathrm{thr}(A+B)$ is symmetric
in its arguments, we concluded that no unit of this class can distinguish the
two cases. \textbf{That argument is wrong, and we withdraw it.} It holds only
...
selectivity. An earlier version of this manuscript reported opposed mirror
pairs from a single configuration; the sweep does not support that claim and we
withdraw it.
...
cause, and the hypothesis we previously drew from it --- that optimization
...
selectivity is not. We do not know why, and we prefer to say so.
...
One methodological caution follows from this work. An earlier draft reported
mean $|\mathrm{DSI}|=0.37$ for \cell{T4a} cells driven by their own recorded
inputs in isolation, and built a causal account on it. That measurement is an
artefact and is withdrawn: the same procedure applied to a \emph{single} input,
where the geometry makes direction selectivity impossible, returns $0.12$
rather than $0$. The cause is that \cell{Mi1} is itself weakly selective
($|\mathrm{DSI}|=0.040$) and a threshold unit amplifies that residue. Isolated
replay is not a clean control for this question.

\subsection{One global gain is provably insufficient}
\label{sec:regional}

The account above stops at a negative result. It should not, because the
obstacle turns out to be a specific and testable property of the model rather
than an unknown, and removing it produces behaviour.

\emph{Shunting inhibition cancels itself.} A correlator needs a multiplicative
interaction between its arms, and in a conductance neuron that must come from
inhibition dividing. Measured at the working point, it does not. Inhibition
divides the membrane by $g_{\mathrm{tot}}$, $2.21\to3.92$, a factor $1.77$; but
the same inhibition drops $v_{\infty}$ from $-23.5$ to $-43.4$\,mV, raising the
excitatory driving force $(E_e-v)$ by $1.85$. The net interaction is
$1.85/1.77=1.056$. Across \cell{T4a}, \cell{T4b}, \cell{T5a} and \cell{T5b} it
never exceeds $11\%$, and its sign is \emph{wrong}: inhibition slightly
increases the excitatory gain. An assumption-free check agrees --- a purely
additive model $v=a+b\,g_e+c\,g_i$ explains $R^2=0.865$ of \cell{T4a}'s
voltage, and adding the bilinear term buys $2.2$ percentage points. The
correlator has been performing addition, which cannot be direction selective at
any wiring.
...
shunt, and only when inhibition substantially exceeds excitation does the
division survive.

\emph{Raising that ratio works, and where it is applied decides everything.}
Doubling every inhibitory synapse in the brain raises direction selectivity
five- to sevenfold and destroys the gustatory result: bitter stops suppressing
the proboscis motor pool and begins driving it at $168$\,Hz. The sign inverts
because $557{,}080$ of $1{,}099{,}675$ inhibitory edges --- $51\%$ --- terminate
on other inhibitory neurons, so a global scalar doubles disinhibition exactly as
hard and the circuit runs backwards.

Applying the identical factor to inhibitory synapses onto the visual
populations alone leaves the gustatory circuit \emph{byte-identical}: sugar
$73.58\pm1.44$\,Hz, bitter $0.00$, $99\%$ suppression, all six acceptance
checks passing, the same numbers to the last digit. \cell{MN9} receives $0.00\%$
of its input synapses from those populations, so a regional parameter has no
path into the circuit at all. Olfaction survives too, including the readout that
is genuinely exposed: \cell{DNp01} draws $20.5\%$ of its input from the scaled
population and holds at $-17.02\pm7.15$\,Hz against $-18.13\pm2.48$ baseline,
with the correct sign and all four checks passing. The one real cost is
\cell{DNa02}, which falls $12.12\to2.65$\,Hz.

Same manipulation, same magnitude, opposite outcome, and the only difference is
\emph{where}. This is the paper's central claim measured rather than argued: a
frozen connectome under one global synaptic gain is not merely incomplete but
inconsistent, because the optic lobe requires an $E/I$ ratio the SEZ cannot
survive. Published physiology puts the optic lobe's requirement in the same
place --- GABAergic conductance is $\sim2.5\times$ cholinergic in two unrelated
preparations, \citet{suodowd2003} measuring both transmitters as quantal events
in the same cells and \citet{groschner2022} fitting adult \cell{T4} in vivo, and
the latter report inhibitory-to-excitatory conductance ratios in adult
lobula-plate neurons of $7.7$ and $10.9$. Our factor of two is at the
conservative end of what that literature supports.

\subsection{Behaviour under a regional parameterisation}

With inhibition onto the visual populations doubled, \cell{T4}/\cell{T5}
spiking, optic gain $16$ and no tonic bias, the model was run against the
command-matched AR(1) control on thirty environment seeds that were not used
while choosing those values.

\begin{center}
\begin{tabular}{lrr}
\toprule
metric (paired difference vs.\ random) & regional & frozen global\\
\midrule
...
Ten of eighteen metrics separate from the control; the frozen model separates
on none. In absolute terms the regional model collects $30.0$ health units
against its matched control's $18.1$, where the frozen model collects $13.2$
against $20.9$. It covers more ground, gets stuck less, and hits fewer walls.

Three controls argue the effect is not an artefact of drive. Reaching the same
firing rate by lowering gain instead gives $-0.0152$ against $-0.0716$, four to
five times short. The identical factor applied to \emph{non}-visual inhibitory
synapses gives $-0.0102$ against an unmanipulated $-0.0107$, i.e.\ nothing. And
on the $1{,}869$ cells driven above threshold in both conditions the projected
selectivity moves $+0.0001\to-0.0720$, so it is not survivorship among a
shrinking population.

Two further controls bound the claim, and we state them because they qualify
it. Freezing the retina on a single frame leaves the collision advantage
significant ($-3.56\pm2.9$), so roughly half of it is the temporal structure of
the motor command outperforming an AR(1) surrogate rather than vision.
Mirroring the retina left--right flips the vision--steering correlation as it
should ($-0.21\to+0.13$) but reduces the behavioural point estimates about
threefold rather than abolishing them. The model became a better \emph{mover}
as well as a better \emph{seer}, and only the second is the claim this paper
makes. The optimum is also sharp --- selectivity collapses by a factor of four
--- which is characteristic of maximising over a one-dimensional slice, and a
structured search over per-receptor and regional parameters is the obvious next
...
($1563\times$), \cell{DNa02} $102\times$, \cell{DNp01} to within noise of zero
--- landing at the $0.00$--$0.11$\,Hz scale of the \cell{LC4} control, that is,
at the network's generic coupling floor.

Sign consistency and effect magnitude discriminate differently here, and only
the second separates the arms. Sign consistency does \emph{not}: several shuffled populations remain fairly
...
25.7--25.8\,Hz across seeds, an $80\times$ higher floor. Preserving degree while destroying specificity does not merely
...
scope precisely. The connectome contains the correlator: we found the geometry,
the mirrored axes, the phase offset and the travelling wave that drives them.
What the frozen model does not supply is whatever additional structure makes
those ingredients function together, and we are careful not to name it. Our
first candidate was the per-type gain that makes the two arms comparable in
magnitude --- in our hands they differ by $37\times$ --- but correcting the
imbalance, by depression and by transplanting an optimized model's gains,
lowered selectivity rather than raising it. The honest summary is that a
simulator demonstrably capable of $\mathrm{DSI}=0.79$ in a hand-built
correlator yields $0.002$ from the same units wired by the connectome, and we
cannot presently say what the difference consists of.

Placed beside \citet{lappalainen2024}, this becomes informative rather than
merely negative. That model shares our wiring and recovers motion selectivity,
but obtains its unknown parameters by optimization. Ours fixes them to a single
published set. The difference between the two outcomes is therefore a
measurement of what the optimization supplies. We took the natural next step
and inspected it: of that model's 604 per-type-pair gains, 571 align with
connections in our graph, and transplanting them lowers our selectivity rather
than raising it. Gain alone, at least as transferred between two different
reconstructions, is not what is missing. What that leaves --- timing structure,
dendritic compartmentalization absent from a point neuron, or an interaction
between gains that does not survive transplantation --- we cannot separate with
the present model.

Two further readings should be excluded. First, none of this is a claim about
whether a fly brain can play a video game; the environment is outside the
animal's evolutionary experience and the model has no ventral nerve cord, so
behavioural competence was never the measurable. Second, the olfaction result
...
\subsection{Connectome data}
 FlyWire FAFB v783~\citep{dorkenwald2024,schlegel2024},
frozen October 2023. Connectivity from the Buhmann synapse
...
\subsection{Neuron model}
 Leaky integrate-and-fire with parameters as listed in
Section~2.1. The subthreshold system
...
\subsection{Departures from the reference model}
 Each is switchable and each
carries a measured justification. (i) \emph{Histamine override} on
...
\subsection{Visual input and the environment}
 Modifying the
game is not a modification of the model, and three properties of Doom are wrong
...
40-tic approach. Every one of these is a common scale on both arms of the
imbalance, which is why the ratio argument predicts the outcome in each case.

\subsection{Rate readout and motor decoding}
 An exponential filter over per-step output.
...
than a single execution suggests. We can put a number on the latter: correcting
an error in the odour-source table --- \texttt{CustomMedikit}, the pickup
actor used by one scenario, was absent from the food list, and unlisted actors
fell through to the pheromone channel, so inanimate scenery was being smelled
as conspecifics --- left the lateral horn response essentially unchanged in
size but moved \cell{DNp01} and \cell{DNa02} from $10/10$ agreeing seeds to
$9/10$. Sign counts near ceiling should therefore be read as $p$ values with
...
\subsection{Tested and deferred}
...
environment into the fly's band; and the engine's own rendering controls. Most of
these share a structure --- each is a scale applied to both arms of a ratio,
and a scale cancels --- but that account is incomplete, because the
\emph{asymmetric} interventions listed below, which do not cancel, failed as
well.
...
\emph{Tested in this round, and also negative:} presynaptic short-term
...
inhibitory flank; global optic-lobe gain to $\times32$ and gain targeted onto
\cell{T4}/\cell{T5} to $\times20$; and matching the network's firing-rate
...
result a failure of the wired model rather than of the method. Rebuilt from
scratch for this revision, the same construction reaches $-0.26$ at one
...
point chosen, and that the qualitative claim --- this model class computes
direction --- is the part the argument rests on.

\emph{A defect in our own instrumentation, disclosed because it invalidates
earlier statements.} The \texttt{-{}-spiking-t4} option was applied in only one
of three code paths; the per-cell and operating-grid paths build their own
graded mask and silently ignored it. Any conclusion about spiking \cell{T4}
drawn from those paths measured the graded model twice, and is withdrawn. The
option is now applied in one place used by all three. Separately, an earlier
per-subtype geometry analysis pooled \cell{Mi9} with \cell{Mi4}, which sit on
\emph{opposite} flanks; averaging them produced a vector that flips with their
relative weight and manufactured both a spurious bimodality and an apparent
$97$--$114^\circ$ mirror-pair defect. Taking \cell{Mi9} alone, the axes are
...
M.U.\"O.\ conceived the study, wrote the simulation and analysis code, ran the
