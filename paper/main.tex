...
tightly and an empty cylinder scores the same, so the apparent fixation is the
decoder's baseline filter and not the eye. Against a size-matched static disc,
...
distinguish an animal that fixates from one that has stopped, and we report it
here as a null with that reason attached rather than as an absence of
behaviour.
