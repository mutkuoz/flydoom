...
preserves olfaction, and increases direction selectivity sevenfold. In closed
loop the same parameterisation changes locomotion measurably against the frozen
model on thirty held-out environment seeds, paired: collisions per thousand tics
$-3.95\pm3.79$ and net displacement $+131\pm102$. It does not, however,
separate from a command-matched control. It differs from that control on six of
eighteen measures, which is the rate the frozen model also achieves, and it
collects less health than its own control. The behavioural measures do not
distinguish the parameterisations.
...
results, and measurably changes closed-loop locomotion against the frozen model
on held-out environment seeds without separating from a command-matched control
by more than the frozen model does.
...
\toprule
parameterisation & values from & proj.\ DSI & differs & better & gustatory & olfactory\\
\midrule
(i) single global gain & fitted on taste & $-0.0107$ & 6/18 & 2/15 & pass & pass\\
(ii) per-receptor, global & published physiology & $-0.0356$ & 4/18 & 2/15 & pass$^{*}$ & \textbf{fail}\\
(iii) uniform $\times2$, global & search & $-0.0533$ & 5/18 & 3/15 & \textbf{fail} & n/a\\
(iv) $\times2$ on vision only & search & $\mathbf{-0.0716}$ & 6/18 & 3/15 & pass & pass\\
\bottomrule
\end{tabular}
...
axis; the raw population mean cancels across cells whose axes oppose. The two
behavioural columns give the number of eighteen metrics on which the model
differs from a command-matched control, and the number of the fifteen
signed metrics on which it differs in the favourable direction, over the same
thirty held-out environment seeds for every row. All four arms fall in the same
narrow band on both counts, the frozen model included, so the behavioural
columns do not discriminate between parameterisations and no row is emphasised
in them; the discrimination in this table is open loop. Row (iii) fails
because bitter stimulation drives the proboscis motor pool to $168$\,Hz instead
of suppressing it. Row (ii) passes the gustatory checks but degraded
($^{*}$sugar $63.79\pm9.31$ against $73.58\pm1.44$\,Hz, suppression $95\%$
against $99\%$) and fails olfaction. Both global assignments break a circuit
the frozen model preserves; only the regional assignment does not.}
\label{tab:threeway}
\end{table}

\emph{(i) A single global gain.} This is the frozen model. Direction
selectivity is $40\times$ below the experimental selection threshold. It is not,
however, behaviourally null: it differs from its command-matched control on six
of eighteen measures, the same count as the regional model
(\S\ref{sec:behaviour}).
...
All four parameterisations were evaluated against the command-matched AR(1)
control on the same thirty environment seeds, none of them used while selecting
parameters, under an identical protocol.

\begin{center}
\small
\begin{tabular}{lrrr}
\toprule
arm & differs from control & better than control & vision--steering $r$\\
\midrule
(i) single global gain     & 6/18 & 2/15 & $-0.090$\\
(ii) per-receptor, global  & 4/18 & 2/15 & $+0.017$\\
(iii) uniform $\times2$    & 5/18 & 3/15 & $-0.031$\\
(iv) $\times2$ vision only & 6/18 & 3/15 & $+0.161$\\
\bottomrule
\end{tabular}
\end{center}

The four arms are indistinguishable on this comparison. The regional model
differs from its control on six of eighteen measures and the frozen model on
the same six-measure count; the directional counts span 2/15 to 3/15 across all
four arms. Neither count orders the arms, and neither orders them by projected
DSI. The frozen model is not the behavioural null the comparison requires: it
separates from a command-matched control at the same rate as the manipulated
model, which means separation from that control does not measure the
manipulation. Health collected, the environment's own objective, moves the
wrong way under (iv): $-7.73$ against its matched control, a smaller quantity
than the frozen model collects.

An earlier version of this section reported ten of eighteen measures for (iv)
and none for (i), a health advantage of $30.0$ units against $18.1$, and a
vision--steering correlation moving from $-0.04$ to $-0.18$. None of the three
survives measurement of all four arms under one protocol on these seeds. The
correlation moves from $-0.090$ to $+0.161$, which is opposite in sign to the
direction previously reported. We withdraw all three.

What does survive is the direct paired comparison between the two
parameterisations, which is the more sensitive test because it removes the
control's variance rather than adding it. Regional against frozen, on the same
thirty seeds, collisions per thousand tics fall by $3.95\pm3.79$ and net
displacement rises by $131\pm102$, both favouring the regional model and both
excluding zero. The parameterisation therefore changes locomotion measurably.
It does not follow, and we do not claim, that it improves task performance: on
the objective the environment scores, it does not.
...
Two further controls were run against the control-referenced comparison before
that comparison was withdrawn, and we report them as such. With the retina
frozen on a single frame the collision advantage persisted ($-3.56\pm2.9$),
and mirroring the retina reversed the vision--steering correlation ($-0.21$ to
$+0.13$) while reducing the behavioural point estimates threefold. Both were
measured under the superseded protocol and neither has been re-run against the
surviving paired comparison; the mirror figures in particular do not reconcile
with the four-arm correlations above. They bound a quantity we no longer claim,
and the reading they supported---that a substantial fraction of the effect
derives from the temporal structure of the motor command rather than from
vision---should be treated as untested rather than as established.
...
The behavioural result is the weakest part of this work, and measuring all four
parameterisations under one protocol on thirty held-out seeds is what
established that. Separation from a command-matched control does not
discriminate: every arm differs from its control on four to six of eighteen
measures and favourably on two to three of fifteen, the frozen model included,
so the control-referenced comparison has no resolving power here and the arm
ordering it appeared to produce was an artefact of measuring one arm. On the
environment's own objective the regional model is worse than its control. What
remains is the direct paired contrast against the frozen model, where
collisions fall and net displacement rises with intervals excluding zero. The
defensible claim is accordingly narrower than we previously made it: removing
an identified obstacle produces a measurable change in locomotion, not an
improvement in task performance, and not evidence that the model sees motion as
a fly does---selectivity remains an order of magnitude below the experimental
selection threshold.

This is the power caution stated in Methods arriving as a result rather than
as a hedge. We noted there that behaviour in this environment is far noisier than
the open-loop rate measurements and that this constrains what any closed-loop
connectome study can claim. Thirty seeds is enough to establish a paired
locomotion difference and not enough to resolve a control-referenced
difference on the objective; the earlier ten-of-eighteen figure is what the
under-powered version of this comparison looks like from the inside. A
connectome model can be shown to change what an embodied agent does long
before it can be shown to make the agent better at anything, and the two should
...
Thirty seeds proved sufficient for the paired contrast between two
parameterisations and insufficient for the contrast between a parameterisation
and a command-matched control, which is the asymmetry the variance argument
predicts: the paired form differences out the seed, the control-referenced form
does not. A control-referenced claim in this environment should be assumed
under-powered at any seed count we can currently afford, and we retract one we
previously made on that basis (\S\ref{sec:behaviour}).
