...
drawn from those paths measured the graded model twice. The option is now
applied in one place used by all three. Separately, an earlier
...
\section*{Note on this revision}

This version corrects four claims made in the previous release, each superseded
by a measurement reported above. The closed-loop rate was given as $1.01\times$
realtime; that figure counted GPU kernel time and omitted dispatch overhead,
and the measured rate is $0.19\times$ (Table~\ref{tab:graph}). Direction
selectivity was attributed to a structural property of the model class via a
symmetry argument that holds only for memoryless units
(Sec.~\ref{sec:mech}). A mean $|\mathrm{DSI}|$ of $0.37$ was reported for
cells driven by their own inputs in isolated replay; the same procedure returns
$0.12$ where the answer must be $0$, and the figure is an artefact of the
method. Mirror-pair opposition was reported from a single operating point and
does not survive the sweep. Two defects in our own instrumentation are
described in Methods: a command-line option applied in one of three code paths,
and a geometry analysis that pooled inputs from opposite dendritic flanks.
